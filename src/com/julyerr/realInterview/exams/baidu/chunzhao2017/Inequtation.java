 ...
 * 1.直接添加到最开始，此时多添加一个>,dp[i-1][j]+1;
 * 2.直接添加到最后面，此时多添加一个<,dp[i-1][j-1]+1;
 * 3.添加到中间任意一个<,例如1<2，变成1<3>2,多添加了一个>，dp[i-1][j]*j;
 * 4.添加到中间任意一个>，例如2>1,变成2<3>1,多添加了一个<,dp[i-1][j-1]*(i-j-1)
